...
class UserController:
    ...
    def _setup_routes(self):
        """라우트 설정"""
        self.router.post("/register", response_model=UserResponse, summary="회원가입")(self.register)
        self.router.post("/login", response_model=TokenResponse, summary="로그인")(self.login)
        self.router.get("/verify", response_model=UserResponse, summary="토큰 검증")(self.verify_token)
        self.router.get("/me", response_model=UserResponse, summary="내 정보 조회")(self.get_current_user_info)
        self.router.post("/logout", summary="로그아웃")(self.logout)
...
# 라우터 추출 (기존 코드와의 호환성을 위해)
auth_router = user_controller.router
